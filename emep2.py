...
pathts2 = (
    input("Path to directory with end files") or "/home/kamil/emep_cut/"
)  # choosing directory with base data and name of the directory where results will be placed
zb = input(
    "Name countries of interest (two or three letters shortcut for each country)"
) or [
    "PL",
    "CZ",
    "SK",
...
otw = Popen(
    ["find", pathts, "-type", "f", "-size", "+0", "-name", "*txt*"], stdout=PIPE
)
...
def ignore_files(dir, files):
    return [
        f for f in files if os.path.isfile(os.path.join(dir, f))
    ]  # defining function that will copy only paths of files


shutil.copytree(pathts, pathts2, ignore=ignore_files)  # copying directory tree
